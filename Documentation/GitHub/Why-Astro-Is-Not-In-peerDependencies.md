...
Both `@playform/favicon` and `@playform/pipe` intentionally keep `astro` in
`dependencies` with a wildcard version (`"*"`) rather than in
`peerDependencies`. This decision follows the precedent established across the
PlayForm integration family after real-world breakage demonstrated that
`peerDependencies` creates more problems than it solves for Astro integrations.
The relevant discussions are tracked in
...
entirely. `@playform/favicon` follows the same approach from the start.
...
- **No version pinning.** A wildcard never pins a specific Astro version, which
  ...
  bundled `astro` and the host project's `astro`. The regression in
  [Compress#583](https://github.com/PlayForm/Compress/issues/583) cannot occur
  in `@playform/favicon`.
...
| -------------------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `dependencies: "astro@5.x.x"` (pinned) | Installs a specific version                              | Pulls mismatched transitive deps; breaks on Astro major version upgrades (see [Compress#583](https://github.com/PlayForm/Compress/issues/583)) |
